...
class ec_paypal extends ec_third_party{
	...
	public function display_form_start( ){
		...
		if( get_option( 'ec_option_paypal_use_selected_currency' ) ){
			...
			echo "<input name=\"amount\" id=\"amount\" type=\"hidden\" value=\"" . esc_attr( $GLOBALS['currency']->convert_price( $this->order->sub_total ) ) . "\" />";
		}else{
			...
			echo "<input name=\"amount\" id=\"amount\" type=\"hidden\" value=\"" . esc_attr( number_format($this->order->sub_total, 2, '.', '' ) ) . "\" />";
		}
		...
		echo "<input name=\"notify_url\" id=\"notify_url\" type=\"hidden\" value=\"".  esc_attr( plugins_url( "wp-easycart/inc/scripts/paypal_payment_complete.php", EC_PLUGIN_DIRECTORY ) ) ."\" />";
