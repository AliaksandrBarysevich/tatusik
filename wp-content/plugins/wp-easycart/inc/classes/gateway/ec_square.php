	...
	*****************************************/
	...
	function insert_option_item( $object, $option_id, $sync = false ){
		if( $this->allowed_at_location( $object ) && !$object->is_deleted ){
			...
			$optionitem_price = ( isset( $object->modifier_data->price_money ) ) ? $object->modifier_data->price_money->amount : -1;
	...
	function insert_product( $object, $sync = false ){
		if( $this->allowed_at_location( $object ) && !$object->is_deleted ){
			...
			$image1 = ( isset( $object->image_id ) ) ? $this->get_image_url( $object->image_id ) : '';
			$option_items = array( );
			...
			if( count( $object->item_data->variations ) > 0 ){
				...
				for( $i=0; $i<count( $object->item_data->variations ); $i++ ){
					...
					if( $this->allowed_at_location( $object->item_data->variations[$i] ) && !$object->item_data->variations[$i]->is_deleted ){
						...
						if( $base_price == 0 || ( isset( $object->item_data->variations[$i]->item_variation_data->price_money ) && $base_price > $object->item_data->variations[$i]->item_variation_data->price_money->amount ) ){
			...
			if( $use_optionitem_quantity_tracking && $option_id ){
				...
				$wpdb->query( $wpdb->prepare( "UPDATE ec_product SET show_stock_quantity = 1, option_id_1 = 0, stock_quantity = %d, square_variation_id = %s WHERE product_id = %d", $stock_quantity, $single_variant_id, $product_id ) );
	...
	function update_product( $object, $product ){
		...
		$category_stripe_id = ( isset( $object->item_data->category_id ) ) ? $object->item_data->category_id : '';
			...
			'changes' => array(
				(object) array(
					...
					'adjustment' => (object) array(
						...
						'to_state' => 'SOLD',
						'location_id' => $location_id,
						'occurred_at' => date( 'Y-m-d', get_option( 'ec_option_square_last_sync' ) ) . 'T' . date( 'H:i:s', get_option( 'ec_option_square_last_sync' ) ) . 'Z',
					),
				),
			),
		);
...
function wp_easycart_square_adjust_option_stock_levels( $quantity, $product_id, $optionitem_id_1, $optionitem_id_2, $optionitem_id_3, $optionitem_id_4, $optionitem_id_5 ) {
	...
	$optionitem = $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM ec_optionitem WHERE optionitem_id = %d', $optionitem_id_1 ) );
	if ( ! $optionitem ) {
		return;
	}
